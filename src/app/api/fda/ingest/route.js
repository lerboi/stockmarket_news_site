// src/app/api/fda/ingest/route.js - Simplified (no type filtering)
...
export async function POST(request) {
  try {
    const { limit = 50, hours = 24 } = await request.json();

    console.log(`Starting FDA RSS ingestion: last ${hours} hours`);

    // Get or create RSS data source
    const dataSource = await ensureRSSDataSource();
    if (!dataSource) {
      throw new Error('Failed to initialize FDA RSS data source');
    }

    // Fetch ALL RSS data (no type filtering - let frontend handle it)
    const response = await fetch(`${getBaseUrl(request)}/api/fda/rss-feed?limit=${limit}&hours=${hours}`);
    if (!response.ok) {
    ...
    if (rssData.length === 0) {
      return NextResponse.json({
        success: true,
        message: `No FDA RSS announcements found in the last ${hours} hours`,
        ingested: 0,
    ...
    console.log(`Retrieved ${rssData.length} FDA RSS announcements, filtering for public companies...`);
    ...
    console.log(`Filtered to ${publicCompanies.length} public companies`);

    // Ingest public companies
    const ingestResults = await Promise.allSettled(
      publicCompanies.map(item => ingestRSSItem(item, dataSource.id))
    );
    ...
    console.log(`RSS ingestion complete: ${successful} successful, ${failed} failed`);
    ...
    console.error('FDA RSS Ingestion Error:', error);
    return NextResponse.json(
      { 
        success: false, 
        error: error.message,
...
async function ensureRSSDataSource() {
  try {
    const { data: existingSource } = await supabase
      .from('data_sources')
      .select('*')
      .eq('source_name', 'FDA RSS Feed')
      .single();
    ...
    console.error('Error ensuring RSS data source:', error);
    return null;
  }
}
...
async function aiFilterCompanyBatch(companyBatch) {
  try {
    const companyList = companyBatch.map((company, index) => 
      `${index + 1}. RSS_ID: "${company.fda_id}", Company: "${company.company_name}", Type: ${company.announcement_type}`
    ).join('\n');

    const prompt = `Analyze these companies from FDA RSS for public trading status.

CRITICAL: Return ONLY a valid JSON array with exactly ${companyBatch.length} objects.

Format: {"fda_id": "exact-id", "company_name": "name", "is_public": true/false, "ticker": "SYMBOL"/null}
...
async function ingestRSSItem(rssItem, sourceId) {
  try {
    const announcementData = {
      ...
      announcement_date: rssItem.announcement_date,
